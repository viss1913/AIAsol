...
async function initDB() {
  try {
    ...
    try {
      await connection.query('SELECT last_generated_image FROM sessions LIMIT 1');
    } catch (e) {
      console.log('⚠️ Column "last_generated_image" missing in sessions. Adding...');
      await connection.query(
        'ALTER TABLE sessions ADD COLUMN last_generated_image MEDIUMTEXT NULL AFTER history'
      );
      console.log('✅ sessions updated with last_generated_image.');
    }

    try {
      await connection.query('SELECT after_reset FROM sessions LIMIT 1');
    } catch (e) {
      console.log('⚠️ Column "after_reset" missing in sessions. Adding...');
      await connection.query(
        'ALTER TABLE sessions ADD COLUMN after_reset TINYINT(1) NOT NULL DEFAULT 0 AFTER last_generated_image'
      );
      console.log('✅ sessions updated with after_reset.');
    }

    try {
      await connection.query('SELECT last_generated_image_at FROM sessions LIMIT 1');
    } catch (e) {
      console.log('⚠️ Column "last_generated_image_at" missing in sessions. Adding...');
      await connection.query(
        'ALTER TABLE sessions ADD COLUMN last_generated_image_at TIMESTAMP NULL AFTER last_generated_image'
      );
